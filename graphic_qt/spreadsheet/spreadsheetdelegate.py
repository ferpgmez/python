...
class SpreadsheetDelegate(QItemDelegate):
    ...
    def createEditor(self, parent, styleOption, index):
        ...
        for i in range(1, index.model().rowCount()):
            strItem = index.model().data(index.sibling(i, index.column()), Qt.ItemDataRole.DisplayRole)
    ...
    def commitAndCloseEditor(self):
        ...
        self.closeEditor.emit(editor, QItemDelegate.EndEditHint.NoHint)
    ...
    def setEditorData(self, editor, index):
        if isinstance(editor, QLineEdit):
            ...
            editor.setDate(QDate.fromString(
                index.model().data(index, Qt.ItemDataRole.EditRole), self.parent().currentDateFormat
            ))
